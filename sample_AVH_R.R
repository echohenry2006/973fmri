...
df$AVH[df$Group=="SZ" & is.na(df$g8)]="NAVH"
...
m.out = matchit(treat ~ Age + Sex + Education, 
                data = df_new, method = "nearest", discard="none",caliper=0.35,
...
rm(df_new)
rm(m.out)
df_new<-subset(df,Filter==site&Race==1&handness==1&(AVH=="NC"|AVH=="AVH"),select=c(Sex,Age,Group,Race,Education,course_year,course_month,handness,FES,Subtype,AVH))
formu<-~Age+Sex+Education
...
rm(df_new)
rm(m.out)
df_new<-subset(df,Filter==site&Race==1&handness==1&(AVH=="NAVH"|(AVH=="AVH"&df$final==1)),select=c(Sex,Age,Group,Race,Education,course_year,course_month,handness,FES,Subtype,AVH))
formu<-~Age+Sex+Education
...
write.xlsx(x = data.frame(df), file=paste0(root_path,"AVH_match_SZ_NC.xlsx"), row.names = FALSE)

# check

df3=read.xlsx2(file=paste0(root_path,"AVH_match_SZ_NC.xlsx"),sheetIndex=1, header=TRUE)
...
for (i in 1:length(site_all)){
  ...
  write.table(df_new$Name[df_new$AVH=="NC"],paste0(root_path,"AVH_match/",site,"_NC.txt"), sep="\n",row.names = F,col.names = F,quote=F)
  write.table(df_new$Name[df_new$AVH=="AVH"],paste0(root_path,"AVH_match/",site,"_AH.txt"), sep="\n",row.names = F,col.names = F,quote=F)
  write.table(df_new$Name[df_new$AVH=="NAVH"],paste0(root_path,"AVH_match/",site,"_NAH.txt"), sep="\n",row.names = F,col.names = F,quote=F)
}
