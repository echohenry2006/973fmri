...
df_fes<-subset(df,Race==1&handness==1&FES!=3,select=c(Filter,Sex,Age,Group,Race,Education,course_year,course_month,handness,FES,Subtype))
...
rm(df_new)
rm(m.out)
df_new<-subset(df,Filter==site&Race==1&handness==1&FES!=3,select=c(Sex,Age,Group,Race,Education,course_year,course_month,handness,FES,Subtype))
...
m.out = matchit(treat ~ Age + Sex , 
                data = df_new, method = "nearest", discard="none",caliper=0.3,
                ratio = 2,reestimate=T) 
summary(m.out) 
# plot(m.out, type = "jitter") 
# plot(m.out, type = "hist")
...
write.xlsx(x = data.frame(df), file=paste0(root_path,"FES_match_SZ_NC.xlsx"), row.names = FALSE)

# check

df3=read.xlsx2(file=paste0(root_path,"FES_match_SZ_NC.xlsx"),sheetIndex=1, header=TRUE)
...
for (i in 1:length(site_all)){
  ...
  write.table(df_new$Name[df_new$Group=="NC"],paste0(root_path,"FES_match/",site,"_NC.txt"), sep="\n",row.names = F,col.names = F,quote=F)
  write.table(df_new$Name[df_new$Group=="SZ"],paste0(root_path,"FES_match/",site,"_SZ.txt"), sep="\n",row.names = F,col.names = F,quote=F)
